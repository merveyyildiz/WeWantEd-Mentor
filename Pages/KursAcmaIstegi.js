...
export default class Kurslar extends Component {
  ...
  fetchData = () => {
    fetch('http://192.168.112.2/wewanted/kurs_acma_istekleri.php')
  ...
  kursaGit = (id) => () => {
    this.props.navigation.navigate('KursAcmaIstegiDetay', { kursId: id });
  };
  componentDidMount() {
    this.fetchData();
  }
    ...
    if (this.state.sayfa === 1) {
      return (
        <View style={styles.istekyokCont}>
          <Text style={styles.istekYok}>Kurs Açma İsteği Bulunmamaktadır.</Text>
        </View>
      );
    } else {
